...
New in v1.1
-----------
  * SMOOTHING: optionally smooth the noise floor for the *drop decision only*
    (your written frequencies still come from the real data). This lets flat,
    scattery regions collapse hard while genuine spikes are still detected
    against the raw curve, so you never lose a real spike.
  * SWEEP_TOLERANCES: emit a reduction/error table across several tolerances in
    one run so you can pick the sweet spot without editing/re-running.
  * PLOT: save a PNG of the full curve with the kept points overlaid so you can
    visually confirm no spike was dropped.
...
def run_sweep(x, y_decide, y_raw, n_total, tolerances, y_units, force_keep=None):
    """Print a reduction/error table across several tolerances."""
    log.info("Tolerance sweep (no output file written):")
    header = f"{'tolerance':>12} | {'kept':>7} | {'reduction':>9} | {'max err':>10}"
    log.info(header)
    log.info("-" * len(header))
    for tol in tolerances:
        ...
        log.info("%12.4g | %7d | %8.2f%% | %8.4g %s",
                 tol, kept, red, err, y_units)
...
def main():
    try:
        ...
        f, x, y_raw = build_axes(f_all, v_all)
        log.info(
        ...
        if SMOOTHING != "none":
            ...
            force_keep = (y_raw - y_decide) > SPIKE_THRESHOLD
            log.info("Smoothing for drop-decision: %s (window=%d). "
                     "Protected %d spike(s) above +%.4g %s floor.",
                     SMOOTHING, SMOOTH_WINDOW, int(force_keep.sum()),
                     SPIKE_THRESHOLD, y_units)
        else:
            y_decide = y_raw

        if SWEEP_TOLERANCES:
            run_sweep(x, y_decide, y_raw, f.size, SWEEP_TOLERANCES, y_units,
